...
//
// Phase 1 (default, READ-ONLY, safe to run anytime): enumerate active layers with DISP_LAYER_GET_CONFIG2
...
//
// NOTE: A133P uses the disp2 config2 ABI (DISP_LAYER_*_CONFIG2 + disp_layer_config2). v1
// returned no layers on-device. NOTE: the disp2 ioctl arg block is 4 kernel-longs {screen, &config, count, 0}. It MUST be
...
static int layer_get(int fd, unsigned scr, struct disp_layer_config2 *cfg) {
	unsigned long args[4] = { scr, (unsigned long)cfg, 1, 0 };
	return ioctl(fd, DISP_LAYER_GET_CONFIG2, args);
}
static int layer_set(int fd, unsigned scr, struct disp_layer_config2 *cfg) {
	unsigned long args[4] = { scr, (unsigned long)cfg, 1, 0 };
	return ioctl(fd, DISP_LAYER_SET_CONFIG2, args);
}

static void print_layer(unsigned scr, const struct disp_layer_config2 *c) {
	const struct disp_fb_info2 *fb = &c->info.fb;
	...
	printf("      crop(SRC)=[%llu,%llu %llux%llu]  dma-buf fd=%d\n",
...
int main(int argc, char **argv) {
	...
	// DEBUG: query the known-active layer (screen 0, ch 1, ly 0 per /sys) and show the raw result,
	// so we can tell "ioctl failed" from "ioctl ok but enable/offsets read wrong".
	{
		struct disp_layer_config2 d; memset(&d, 0, sizeof d);
		d.channel = 1; d.layer_id = 0; d.enable = 1;
		errno = 0;
		int r = layer_get(fd, 0, &d);
		printf("DEBUG scr0/ch1/ly0: ret=%d errno=%d(%s)  enable=%d ch=%u ly=%u  fmt=0x%x mode=%d win=%dx%d\n",
		       r, errno, strerror(errno), d.enable, d.channel, d.layer_id,
		       d.info.fb.format, d.info.mode, d.info.screen_win.width, d.info.screen_win.height);
	}

	// ---- Phase 1: enumerate active layers (READ-ONLY) ----
	struct disp_layer_config2 found;   // first enabled layer we see
	int have_found = 0, active = 0;
	printf("\n=== Phase 1: active layers (DISP_LAYER_GET_CONFIG2) ===\n");
	for (unsigned scr = 0; scr < 2; scr++) {
		for (unsigned ch = 0; ch < 4; ch++) {
			for (unsigned ly = 0; ly < 4; ly++) {
				struct disp_layer_config2 c;
				memset(&c, 0, sizeof c);
				c.channel = ch; c.layer_id = ly; c.enable = 1; // m21 sets enable before GET
	...
	if (!active) {
		printf("  (config2 GET returned no enabled layers across screen 0-1, ch 0-3, ly 0-3 —\n");
		printf("   unexpected, since /sys shows an active layer. May need a wider scan or SET-first.)\n");
		close(fd); return 2;
	}
	...
	struct disp_layer_config2 orig;
	...
	struct disp_layer_config2 test = orig;
	...
	if (layer_set(fd, 0, &test) < 0) {
		fprintf(stderr, "DISP_LAYER_SET_CONFIG2 failed: %s\n", strerror(errno));
		close(fd); return 4;
	}
