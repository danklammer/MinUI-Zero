...
// Deterministic ABORTING-overflow proof. Random pressure often never enters this path,
// so manufacture a full stream, hold the consumer inside its first callback, land a park,
// then emit enough commands (including a barrier) to force the CORE-local overflow ledger.
typedef struct {
	fr_ring fr;
	pthread_mutex_t gate_lk;
	pthread_cond_t gate_cv;
	int core_go, consumer_go;
	_Atomic int stream_full, callback_entered, overflow_done, park_done;
	_Atomic uint32_t overflow_returns;
	uint32_t next_index, commands_applied;
} overflow_case;
static overflow_case ov;

static void ov_wait(_Atomic int* flag, const char* what) {
	struct timespec t0; clock_gettime(CLOCK_MONOTONIC,&t0);
	while (!atomic_load(flag)) {
		struct timespec now; clock_gettime(CLOCK_MONOTONIC,&now);
		if (now.tv_sec - t0.tv_sec > 5) { fprintf(stderr,"overflow-test hung: %s\n",what); abort(); }
		struct timespec nap={0,200000}; nanosleep(&nap,NULL);
	}
}
static void ov_event(void* ctx, const fr_event* ev) {
	(void)ctx;
	if (ev->kind != FR_EV_CMD) return;
	if (!atomic_exchange(&ov.callback_entered,1)) {
		pthread_mutex_lock(&ov.gate_lk);
		while (!ov.consumer_go) pthread_cond_wait(&ov.gate_cv,&ov.gate_lk);
		pthread_mutex_unlock(&ov.gate_lk);
	}
	assert(ev->index == ov.next_index && "ABORTING overflow reordered a command");
	assert(ev->payload == ev->index && "ABORTING overflow corrupted a command payload");
	ov.next_index++;
	ov.commands_applied++;
}
static void* ov_core(void* arg) {
	(void)arg;
	uint64_t op=0, gen=0, snap[4]; uint32_t slot=0;
	assert(fr_core_service_next(&ov.fr,&op) == FR_RELEASED);
	assert(fr_core_wait_grant(&ov.fr,&gen,&slot,snap) == FR_GRANT);
	for (uint32_t i=0;i<FR_STREAM_CAP;i++)
		assert(fr_core_emit(&ov.fr,FR_EV_CMD,FR_EVF_PERSISTENT,i) == FR_OK);
	atomic_store(&ov.stream_full,1);
	pthread_mutex_lock(&ov.gate_lk);
	while (!ov.core_go) pthread_cond_wait(&ov.gate_cv,&ov.gate_lk);
	pthread_mutex_unlock(&ov.gate_lk);
	for (uint32_t i=FR_STREAM_CAP;i<FR_STREAM_CAP+8;i++) {
		uint32_t flags=FR_EVF_PERSISTENT;
		if (i==FR_STREAM_CAP+7) flags|=FR_EVF_BARRIER;
		fr_rc rc=fr_core_emit(&ov.fr,FR_EV_CMD,flags,i);
		assert(rc==FR_OK || rc==FR_ABORT);
		if (rc==FR_ABORT) atomic_fetch_add(&ov.overflow_returns,1);
	}
	assert(fr_core_barrier_wait(&ov.fr) == FR_ABORT);
	atomic_store(&ov.overflow_done,1);
	assert(fr_core_run_done(&ov.fr,FR_OUT_NONE,0) == FR_PARKED);
	assert(fr_core_service_next(&ov.fr,&op) == FR_STOP);
	fr_core_thread_exit(&ov.fr);
	return NULL;
}
static void* ov_park(void* arg) {
	(void)arg;
	fr_park(&ov.fr,ov_event,NULL,FR_DRAIN_NORMAL);
	atomic_store(&ov.park_done,1);
	return NULL;
}
static void deterministic_overflow_test(void) {
	memset(&ov,0,sizeof(ov));
	pthread_mutex_init(&ov.gate_lk,NULL);
	pthread_cond_init(&ov.gate_cv,NULL);
	fr_init(&ov.fr,1); ov.fr.failclosed_sec=60;
	pthread_t core,park;
	assert(!pthread_create(&core,NULL,ov_core,NULL));
	fr_release(&ov.fr);
	while (fr_get_state(&ov.fr)!=FR_RUNNING) { struct timespec n={0,200000}; nanosleep(&n,NULL); }
	uint64_t snap[4]={1,2,3,4};
	assert(fr_grant(&ov.fr,snap,NULL)==FR_OK);
	ov_wait(&ov.stream_full,"producer did not fill stream");
	assert(!pthread_create(&park,NULL,ov_park,NULL));
	ov_wait(&ov.callback_entered,"park drain did not enter callback");
	pthread_mutex_lock(&ov.gate_lk); ov.core_go=1; pthread_cond_broadcast(&ov.gate_cv); pthread_mutex_unlock(&ov.gate_lk);
	ov_wait(&ov.overflow_done,"producer did not enter ABORTING overflow");
	ASSERT_INV(25,atomic_load(&ov.overflow_returns)>0,"deterministic case missed ABORTING command overflow");
	pthread_mutex_lock(&ov.gate_lk); ov.consumer_go=1; pthread_cond_broadcast(&ov.gate_cv); pthread_mutex_unlock(&ov.gate_lk);
	pthread_join(park,NULL);
	ASSERT_INV(26,atomic_load(&ov.park_done),"forced park did not complete");
	ASSERT_INV(27,ov.commands_applied==FR_STREAM_CAP+8,"ABORTING overflow lost a command");
	ASSERT_INV(28,atomic_load(&ov.fr.barrier_applied)==atomic_load(&ov.fr.barrier_emitted),
	           "ABORTING overflow barrier ledger did not settle");
	fr_stop(&ov.fr);
	pthread_join(core,NULL);
	fr_drain(&ov.fr,ov_event,NULL,FR_DRAIN_NORMAL);
	fr_destroy(&ov.fr);
	pthread_cond_destroy(&ov.gate_cv);
	pthread_mutex_destroy(&ov.gate_lk);
}
...
int main(int argc, char** argv) {
	...
	fr_release(&h.fr);
	// Deterministic maximum-depth exercise: prove all four credits/slots can be live.
	fr_set_depth(&h.fr, 4, drain_cb_park, NULL, FR_DRAIN_NORMAL);
	...
	for (;;) {
		...
		uint32_t credits = fr_credits_out(&h.fr);
		if (credits > max_credits) max_credits = credits;
	...
	ASSERT_INV(24, atomic_load(&h.fr.svc_seq) == atomic_load(&h.svc_ops_done),
	           "svc_seq mismatch");
	ASSERT_INV(29, max_credits == 4, "depth-4 never held all four credits");
