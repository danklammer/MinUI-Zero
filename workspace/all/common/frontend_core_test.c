...
static _Atomic uint64_t g_runtime_svc_cmd_applied = 0; // INV27: runtime service products reach the integrator as commands
...
static int  fk_unserialize(void* c, uint64_t a){
	(void)a; fakecore* fk=c;
	// A real unserialize may publish geometry/AV changes. Exercise the service transport
	// class so an integrator cannot accidentally apply only run-epoch FR_EV_CMD events.
	fc_emit_cmd(fk->f, FR_EVF_PERSISTENT, 0x5100);
	fc_emit_barrier(fk->f, 0x5101);
	return (rng_below(&fk->run_rng,20)==0)?-1:0;
}
static void fk_reset      (void* c){ (void)c; }
static int  fk_runtime    (void* c, uint64_t a){ (void)a; assert_affinity(FC_OP_RUNTIME); return stage_rc(c, FC_OP_RUNTIME); }
...
static void fill_vtable(fc_vtable* vt, fakecore* fk){
	...
	vt->run=fk_run; vt->serialize=fk_serialize; vt->unserialize=fk_unserialize; vt->reset=fk_reset; vt->runtime_service=fk_runtime;
...
static void on_ev(void* ctx, const fr_event* ev){
	applied_events++;
	if (ev->kind == FR_EV_SERVICE) {
		assert(fr_event_is_command(ev) && "runtime service product must be command-applicable");
		if (ev->payload == 0x5100 || ev->payload == 0x5101)
			atomic_fetch_add(&g_runtime_svc_cmd_applied, 1);
	}
	if (ctx){
		drainrec* d = ctx;
		d->events++;
...
static fc_state boot_stage_by_stage(fc* f){
	...
	for (unsigned i=0;i<sizeof(seq)/sizeof(seq[0]);i++){
		fc_boot_stage(f, seq[i], on_ev, NULL);
...
static void run_session(int fail_op, int fail_kind, rng_t* mr, int stage_by_stage){
	...
	if (!failed){
		// A recoverable runtime operation failure must be returned to MAIN without poisoning
		// bootstrap/lifecycle state. Otherwise one failed save silently turns every later stage
		// into a failed-bootstrap session.
		fk.fail_op = FC_OP_RUNTIME; fk.fail_kind = -1;
		fc_state before_runtime_fail = fc_get_state(&F);
		INV(25, fc_menu_op(&F, FC_OP_RUNTIME, 0, on_ev, NULL) == -1,
		    "runtime service failure was not returned to MAIN");
		INV(26, !fc_boot_failed(&F) && fc_get_state(&F) == before_runtime_fail,
		    "runtime service failure poisoned lifecycle state");
		fk.fail_op = 0; fk.fail_kind = 0;
		// Deterministic coverage for INV27; do not rely on the randomized service mix
		// happening to choose an unserialize in this run.
		(void)fc_menu_op(&F, FC_OP_UNSERIALIZE, 0, on_ev, NULL);
		...
		for(;;){
			...
			else if (roll < 32){ fc_menu_op(&F, FC_OP_RUNTIME, rng_below(mr,8), on_ev, NULL); }
			else if (roll < 34){
				fc_park(&F,on_ev,NULL,0);
				INV(23, fr_get_state(&F.fr)==FR_QUIESCENT, "explicit menu park did not quiesce CORE");
				fc_menu_op(&F, FC_OP_RUNTIME, rng_below(mr,8), on_ev, NULL);
				INV(24, fr_get_state(&F.fr)==FR_QUIESCENT, "runtime service released an outer menu park");
				fc_release(&F);
			}
		}
	}
...
static void run_cancel_test(rng_t* mr){
	...
	// fc_boot_finish publishes an asynchronous release intent. The shipping MAIN loop retries a
	// nonblocking pump until CORE consumes it; this subtest issues exactly one pump, so first wait
	// for the documented RUNNING edge before testing mid-epoch stop cancellation.
	poll_until(&F.fr.state, FR_RUNNING, "CORE never consumed bootstrap release");

	atomic_store(&g_pump_returned, 0);
	pthread_t pt;
	if (pthread_create(&pt,NULL,pump_thread_fn,&F)) { perror("pthread_create"); abort(); }
...
// A stopped engine must reject a late menu/service request before touching the service
// queue. This is the integration guard that keeps fr_service's wait contract unreachable
// after CORE exit.
static void run_stopped_service_test(rng_t* mr){
	fakecore fk; memset(&fk,0,sizeof(fk));
	fk.run_rng.s = rng_next(mr) | 1;
	fc_vtable vt; fill_vtable(&vt,&fk);
	fc F; fk.f=&F;
	fc_init(&F,&vt,1);
	poll_until(&F.fr.state, FR_QUIESCENT, "CORE did not enter initial quiescence");
	fr_stop(&F.fr);
	poll_until(&F.fr.state, FR_STOPPED, "CORE did not stop before late service test");
	INV(28, fc_menu_op(&F, FC_OP_RUNTIME, 0, on_ev, NULL) == -1,
	    "fc_menu_op accepted a service after STOPPED");
	fc_teardown(&F);
}

// At depth two, one non-blocking pump may observe both outstanding epoch boundaries.
// The integration and governor reserve capacity for this exact composition; assert it
// deterministically instead of relying on scheduler luck in randomized sessions.
static void run_multi_rundone_test(rng_t* mr){
	fakecore fk; memset(&fk,0,sizeof(fk));
	fk.run_rng.s = rng_next(mr) | 1;
	fk.quiet_run = 1;
	fc_vtable vt; fill_vtable(&vt,&fk);
	...
	fc_init(&F,&vt,2);
	fc_state reached = fc_bootstrap(&F);
	INV(29, reached == FC_RESUME_APPLIED || reached == FC_RUNNING,
	    "multi-RUN_DONE bootstrap did not reach RUNNING");
	poll_until(&F.fr.state, FR_RUNNING, "CORE never consumed depth-2 bootstrap release");
	for (int i=0;i<2;i++) {
		uint64_t snap[4], gen = atomic_load(&F.fr.gen) + 1;
		mk_snap(gen, snap);
		assert(fr_grant(&F.fr, snap, NULL) == FR_OK);
	}
	struct timespec d0; clock_gettime(CLOCK_MONOTONIC,&d0);
	while (atomic_load(&F.fr.rd_tail) - atomic_load(&F.fr.rd_head) < 2) {
		struct timespec n; clock_gettime(CLOCK_MONOTONIC,&n);
		if (n.tv_sec - d0.tv_sec > 5) { fprintf(stderr,"multi-RUN_DONE production hung\n"); abort(); }
		struct timespec s={0,200000}; nanosleep(&s,NULL);
	}
	drainrec dr={0}; uint64_t snap[4]; mk_snap(atomic_load(&F.fr.gen)+1, snap);
	fc_pump(&F, snap, on_ev, &dr); // credits are full, so this pump only drains
	INV(30, dr.rundone == 2, "one depth-2 pump did not drain both published RUN_DONE events");
	fc_teardown(&F);
}
...
int main(int argc, char** argv){
	...
	run_stopped_service_test(&mr);
	run_multi_rundone_test(&mr);
	...
	// INV27: runtime services use FR_EV_SERVICE rather than FR_EV_CMD. The integration's
	// command dispatcher must recognize both or a load-state AV barrier can release while
	// MAIN silently ignores the geometry/timing payload.
	INV(27, atomic_load(&g_runtime_svc_cmd_applied) > 0, "runtime service command was not applied");
