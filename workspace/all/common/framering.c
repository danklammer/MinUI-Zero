...
// other side has announced it is (or may be about to be) sleeping. Monotonic wake
// generations catch ordinary signal-before-wait crossings without putting a lock in
// the steady path. Because those counters remain acquire/release rather than seq_cst,
// the bounded timed waits below are still the formal liveness backstop on weak memory.
...
static void wake_cons(fr_ring* fr) {
	atomic_fetch_add_explicit(&fr->cons_wake_seq, 1, memory_order_release);
...
static void wake_prod(fr_ring* fr) {
	atomic_fetch_add_explicit(&fr->prod_wake_seq, 1, memory_order_release);
...
static void prod_sleep(fr_ring* fr, fr_pred pred, int drain_driven, const char* where) {
	...
	pthread_mutex_lock(&fr->lk);
	uint64_t seen = LD(fr->prod_wake_seq);
	ST(fr->prod_waiting, 1);
	while (!pred(fr)) {
		uint64_t now = LD(fr->prod_wake_seq);
		if (now != seen) {
			seen = now;
			continue;
		}
...
static int pred_cons_events(fr_ring* fr) {
...
void fr_wait_events(fr_ring* fr) {
	...
	pthread_mutex_lock(&fr->lk);
	uint64_t seen = LD(fr->cons_wake_seq);
	if (!pred_cons_something(fr)) {
		ST(fr->cons_waiting, 1);
		if (!pred_cons_something(fr) && LD(fr->cons_wake_seq) == seen) {
			struct timespec ts;
			clock_gettime(CLOCK_MONOTONIC, &ts);
...
void fr_service(fr_ring* fr, uint64_t op, fr_drain_cb cb, void* ctx) {
	...
	for (;;) {
		...
		if (!(LD(fr->svc_acked) >= want && LDRLX(fr->sv_head) == LD(fr->sv_tail))
		 && !pred_cons_events(fr) && LD(fr->cons_wake_seq) == seen) {
			struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts); ts.tv_sec += 1;
			pthread_cond_timedwait(&fr->cv_cons, &fr->lk, &ts);
		}
		ST(fr->cons_waiting, 0);
...
fr_rc fr_core_barrier_wait(fr_ring* fr) {
	...
	for (;;) {
		...
		prod_sleep(fr, pred_barrier, 1, "barrier_wait");
	}
}
...
fr_rc fr_core_service_next(fr_ring* fr, uint64_t* out_op) {
	...
	for (;;) {
		...
		uint64_t seen = LD(fr->prod_wake_seq);
		ST(fr->prod_waiting, 1);
		if (!pred_service(fr) && LD(fr->prod_wake_seq) == seen) {
			struct timespec ts; clock_gettime(CLOCK_MONOTONIC, &ts); ts.tv_sec += 1;
			pthread_cond_timedwait(&fr->cv_prod, &fr->lk, &ts);
		}
		ST(fr->prod_waiting, 0);
