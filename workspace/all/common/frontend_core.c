...
static void dispatch_service(fc* f, uint64_t opword) {
	...
	case FC_OP_RUNTIME:     r = f->vt.runtime_service(f->vt.ctx, arg); break;
	default: break;
	}
	tl_phase = FCP_NONE;
	...
	// Bootstrap failure terminates bootstrap. Runtime menu/service failures are reported to
	// fc_menu_op through last_op_result but must not poison the lifecycle state: a failed save
	// or disc operation does not make subsequent CORE work a failed bootstrap.
	if (r && op <= FC_OP_RESUME) {
...
void fc_init(fc* f, const fc_vtable* vt, uint32_t depth) {
	...
	if (pthread_create(&f->core_thread, NULL, fc_core_thread, f) == 0) {
		...
		f->thread_started = 0;
		atomic_store_explicit(&f->boot_failed, 1, memory_order_release);
		// No worker can own the ring, so close its lifecycle and release the
		// mutex/condvars here. fc_teardown is intentionally a no-op without a thread.
		fr_core_thread_exit(&f->fr);
		fr_destroy(&f->fr);
	}
}
...
// keep issuing stages. CORE-side stages drain through the caller callback so persistent
// geometry/AV products emitted by load or auto-resume are actually applied on MAIN.
// Returns the state reached; check fc_boot_failed() after.
fc_state fc_boot_stage(fc* f, fc_op op, fr_drain_cb cb, void* cbctx) {
	...
	if (op == FC_OP_AUDIO || op == FC_OP_RENDERER)
		...
		fr_service(&f->fr, (uint64_t)op, cb_or_noop(cb), cbctx); // CORE-side
	return fc_get_state(f);
}
...
fc_state fc_bootstrap(fc* f) {
	...
	for (unsigned i = 0; i < sizeof(seq)/sizeof(seq[0]); i++) {
		fc_boot_stage(f, seq[i], NULL, NULL);
		if (atomic_load_explicit(&f->boot_failed, memory_order_acquire))
...
void fc_pump(fc* f, const uint64_t snapshot[4], fr_drain_cb cb, void* cbctx) {
	...
	if (rc == FR_OK && f->fr.depth <= 1) {
		...
		// depth >= 2 remains non-blocking so CORE can overlap the previous present. The
		// integration's vsynced frame drain is the primary pacer and its bounded no-progress
		// sleep prevents a free-running MAIN poll loop.
...
int fc_menu_op(fc* f, fc_op op, uint64_t arg, fr_drain_cb cb, void* cbctx) {
	if (arg > (UINT64_MAX >> 8)) return -1;
	// Always establish a fresh park boundary before enqueueing the service. fr_release is an
	// asynchronous intent, so observing QUIESCENT alone is insufficient: a preceding shortcut
	// may have a release pending that flips CORE to RUNNING immediately after the observation.
	// Capture that pending intent under the lifecycle lock; if it exists, this operation inherits
	// the obligation to release. An outer menu park has neither RUNNING state nor release intent,
	// so services issued from the menu preserve its QUIESCENT ownership.
	pthread_mutex_lock(&f->fr.lk);
	fr_state before = fr_get_state(&f->fr);
	int release = before == FR_RUNNING
	           || atomic_load_explicit(&f->fr.release_req, memory_order_acquire);
	pthread_mutex_unlock(&f->fr.lk);
	if (before == FR_STOPPED) return -1;
	fr_park(&f->fr, cb_or_noop(cb), cbctx, FR_DRAIN_NORMAL);
	if (fr_get_state(&f->fr) == FR_STOPPED) return -1;
