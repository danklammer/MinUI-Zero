...
[ -z "$(ls -A "$R/.minui-staging" 2>/dev/null)" ] && ok "no staging tree left behind" || bad "staging tree left on card"
...
# THE POINT OF STAGING: a failed extract must not have touched the live system at all.
[ "$(cat "$R/.system/miyoomini/paks/MinUI.pak/launch.sh")" = "OLD" ] && ok "live .system untouched by a failed extract" || bad "live .system was modified by a failed extract"
rm -rf "$R"
...
[ "$(cat "$R/.system/miyoomini/paks/MinUI.pak/launch.sh")" = "OLD" ] && ok "live .system untouched by a bad payload" || bad "live .system modified by a bad payload"
rm -rf "$R"
