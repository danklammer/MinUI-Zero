# Threading v2.4 — ownership-model design (v1.4)

Status: CONTRACT PHASE CLOSED (see final section). Resolves round-4's four semantic P0s
...
emulation/present overlap; the valid A-arm. **Threaded mode is `depth > 1`** (v1.4 ships
depth 2): CORE runs N+1 while MAIN drains N; RUN_DONE queue and snapshot array are sized
