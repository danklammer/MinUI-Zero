#!/bin/sh
# h700 SFC pak — hosted-dev. minarch + gambatte, measured panel rate, pipewire audio route.
...
"$SYSTEM_PATH/bin/minarch.elf" "$CORES_PATH/snes9x2005_plus_libretro.so" "$ROM" > "$LOGS_PATH/$EMU_TAG.txt" 2>&1
