#!/bin/sh
# h700 SUPA pak — hosted-dev. minarch + gambatte, measured panel rate, pipewire audio route.
...
"$SYSTEM_PATH/bin/minarch.elf" "$CORES_PATH/mednafen_supafaust_libretro.so" "$ROM" > "$LOGS_PATH/$EMU_TAG.txt" 2>&1
