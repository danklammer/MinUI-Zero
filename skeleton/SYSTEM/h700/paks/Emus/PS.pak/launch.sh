#!/bin/sh
# h700 PS pak — hosted-dev. minarch + gambatte, measured panel rate, pipewire audio route.
...
"$SYSTEM_PATH/bin/minarch.elf" "$CORES_PATH/pcsx_rearmed_libretro.so" "$ROM" > "$LOGS_PATH/$EMU_TAG.txt" 2>&1
