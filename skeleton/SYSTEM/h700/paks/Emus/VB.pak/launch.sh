#!/bin/sh
# h700 VB pak — hosted-dev. minarch + gambatte, measured panel rate, pipewire audio route.
...
"$SYSTEM_PATH/bin/minarch.elf" "$CORES_PATH/mednafen_vb_libretro.so" "$ROM" > "$LOGS_PATH/$EMU_TAG.txt" 2>&1
